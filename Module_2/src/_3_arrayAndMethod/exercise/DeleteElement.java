...
public class DeleteElement {
    public static void main(String[] args) {
        DeleteElement myObject = new DeleteElement();
        ...
        if(myObject.search_indexElement(arr,search) != -1 ) {
            for (int j = myObject.search_indexElement(arr, search); j < arr.length; j++) {
                if( j != arr.length -1) {
                    arr[j] = arr[j + 1];
    ...
    public int search_indexElement(int[] array,int search) {
        int index= -1;
